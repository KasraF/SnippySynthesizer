...
class ParserTests extends JUnitSuite
{
	...
	@Test def parseListWithEllipsis1(): Unit = assertEquals(Some(List(1, Ellipsis, 3)), parser.parse(" [ 1 , ..., 3 ] "))
	@Test def parseListWithEllipsis2(): Unit = assertEquals(Some(List(Ellipsis, 2, 3)), parser.parse(" [ ... , 2, 3 ] "))
	@Test def parseListWithEllipsis3(): Unit = assertEquals(Some(List(1, 2, Ellipsis)), parser.parse(" [ 1 , 2, ...] "))
	@Test def parseListWithEllipsis4(): Unit = assertEquals(Some(List("HP", "R...", Ellipsis)), parser.parse("['HP', 'R...', ...]"))
